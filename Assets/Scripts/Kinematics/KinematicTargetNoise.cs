...
public class KinematicTargetNoise : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Quaternion rotation = Quaternion.AngleAxis(angle, Vector3.forward); // rotate around the z axis 
        Vector3 localPosition = rotation * (Vector3.right * length) + (Vector3)noiseOffset;
        Vector3 position = transform.parent.position + localPosition; // moves around the parent position
        //Vector3 position = localPosition; // moves around the world position
