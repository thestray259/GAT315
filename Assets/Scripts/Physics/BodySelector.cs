...
public class BodySelector : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if (body != null || selectedBody != null)
		{
			...
			highlight.transform.position = body.position;
			highlight.transform.rotation = Quaternion.AngleAxis(Time.time * 90, Vector3.forward); // rotate around z axis 
		...
		if (selectedBody != null)
		{
			...
			if (selectedBody.bodyType == Body.eBodyType.DYNAMIC)
			{
				...
				Vector2 force = Spring.Force(target, selectedBody.position, 0, 10);
